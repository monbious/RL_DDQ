...
if __name__ == "__main__":
    ...
    for i in range(len(path_list)):
        ...
        turns = []
        for j in range(3):
            path_item = path_list[i]
            path = './deep_dialog/checkpoints/' + path_item + str(j+1) + '/agt_' \
                   + ('10' if 'DQNZ' in path_item else '6') + '_performance_records.json'
        ...
        plt.plot(x[:-1], sr_mean[i][:-1], color=color_list[i], label=labels[i], lw=0.6)
        plt.fill_between(x[:-1], sr_mean[i][:-1]+sr_std[i][:-1], sr_mean[i][:-1]-sr_std[i][:-1], color=sub_color[i], alpha=0.2)

    plt.legend(loc='lower right', prop={'size': 6})
    plt.savefig('res_success_mcts1.png')
    # plt.show()
